...
3. **Source-generated** (`Chatter.CQRS.SourceGenerated.GeneratedHandlerRegistration.RegisterAll(services)` / `GeneratedAllCommandsBehaviorRegistration.RegisterAll(services)` / `Chatter.MessageBrokers.SourceGenerated.GeneratedReceiverRegistration.RegisterAll(services)`) — same AOT safety as manual explicit, but the calls are generated for you at build time. Referencing `Chatter.CQRS` at all is enough — the generator ships embedded as an analyzer inside `Chatter.CQRS.nupkg`, no extra `PackageReference`. Handler discovery needs no marker attribute at all; `[RegisterForAllCommands]` is needed on a generic behavior class (see `Chatter.Samples.Cqrs`'s `LoggingCommandBehavior<TCommand>`) to have it applied to every discovered command; receiver discovery needs `[BrokeredMessage(...)]` on the message type itself (the exact attribute the reflection-based scan already keys off) — see `Chatter.Samples.RabbitMq.Aot`'s `PingSent`. Message-broker receivers are covered as of the #408 follow-up; only the `InfrastructureType` isn't attribute-driven, since `BrokeredMessageAttribute`'s constructor arguments must be compile-time constants and infrastructure marker constants (e.g. `RabbitMqMessageContext.InfrastructureType`) are `static readonly`, not `const` — leave it unset and it falls back to whichever single infrastructure is registered, which is what `Chatter.Samples.RabbitMq.Aot` does.
...
3. **`GeneratedReceiverRegistration.RegisterAll(services)` instead of a hand-written `AddQueueReceiver<TMessage>()` call** — `PingSent` carries `[BrokeredMessage(sendingPath: PingQueue.Name, receivingPath: PingQueue.Name, deadletterQueueName: PingQueue.DeadLetterName)]` instead, the exact attribute the reflection-based scan already keys off, and the generator discovers it at build time. `InfrastructureType` is deliberately left off the attribute — `BrokeredMessageAttribute`'s constructor arguments must be compile-time constants, and `RabbitMqMessageContext.InfrastructureType` is `static readonly`, not `const`, so it can't be referenced there. Left unset, `ReceiverOptions.InfrastructureType` falls back to whichever single infrastructure is registered — verified directly (both JIT and this Native AOT publish log `Setting default IMessagingInfrastructure to 'Chatter.Infrastructure.RabbitMQ'` and complete a real round trip).
