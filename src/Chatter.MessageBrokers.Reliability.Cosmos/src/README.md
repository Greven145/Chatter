...
`IsAotCompatible` is set for this module, but the Azure Cosmos DB .NET SDK (`Microsoft.Azure.Cosmos`, v3.61.0 as referenced here) carries a real, upstream Newtonsoft.Json dependency outside Chatter's control: it is not declared as a NuGet package dependency (confirmed absent from this module's `packages.lock.json`), but the SDK's own client assembly references Newtonsoft.Json types internally (`NewtonsoftToCosmosDBReader`/`CosmosDBToNewtonsoftReader`/`NewtonsoftSettings`, confirmed by inspecting `Microsoft.Azure.Cosmos.Client.dll`) — reflection-heavy code the trim analyzer cannot see or warn about, since it lives inside a third-party assembly not annotated for trimming. This is a permanent, upstream limitation this initiative does not attempt to solve.
